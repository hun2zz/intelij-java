...
public class Main {
    public static void main(String[] args) {
        ...
        List<Double> collect = bookings.stream().filter(d -> d.getYear() == 2022)
                .map(d -> d.getPrice()).sorted().collect(Collectors.toList());
        System.out.println("sorted = " + collect);

        // 연습 2: 여행지가 있는 모든 나라 이름을 중복 없이 나열하시오.
        List<String> collect1 = bookings.stream().map(d -> d.getDestination().getCountry()).distinct()
                .collect(Collectors.toList());
        System.out.println("collect1 = " + collect1);

        // 연습 3: Paris에 예약된 모든 여행자를 찾아 이름순으로 오름차 정렬.
        List<Booking> paris1 = bookings.stream().filter(d -> d.getDestination().getCity().equals("Paris"))
                .sorted(Comparator.comparing(d -> d.getTraveler().getName()))
                .collect(Collectors.toList());

        System.out.println("paris1 = " + paris1);

        // 연습 4: 모든 여행자의 이메일을 리스트에 모아서 알파벳순으로 오름차정렬하여 반환.
        List<String> collect2 = bookings.stream().map(d -> d.getTraveler().getEmail()).sorted().distinct().collect(Collectors.toList());
        System.out.println("collect2 = " + collect2);

        // 연습 5: Italy로의 여행이 한 건이라도 있는지 여부 확인.
        boolean b = bookings.stream().anyMatch(d -> d.getDestination().getCountry().equals("Italy"));
        System.out.println("b = " + b);

        // 연습 6: France로의 모든 여행의 총 가격을 출력.
        int sum = bookings.stream().filter(d -> d.getDestination().getCountry().equals("France"))
                .mapToInt(d -> (int) d.getPrice())
                .sum();
        System.out.println("sum = " + sum);

        // 연습 7: 예약된 모든 여행에서 최고 가격은 얼마인가?
        List<Double> collect3 = bookings.stream().map(d -> d.getPrice()).sorted(Comparator.reverseOrder())
